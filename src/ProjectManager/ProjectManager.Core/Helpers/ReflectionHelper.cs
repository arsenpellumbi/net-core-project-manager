...
namespace ProjectManager.Core.Helpers
{
    public static class ReflectionHelper
    {
        ...
        public static object GetPrivateFieldValue(object target, string fieldName)
        {
            ...
            if (fi == null)
            {
                throw new ArgumentNullException($"Field '{fieldName}' not found in type hierarchy.");
            }
            ...
            bool onlyConcreteClasses = true, bool ignoreReflectionErrors = false)
        {
            ...
            assemblies.ThrowIfNull(nameof(assignTypeFrom));
        ...
        public static IEnumerable<Type> GetClassesOfType(Type assignTypeFrom, Assembly assembly, bool onlyConcreteClasses = true, bool ignoreReflectionErrors = false)
        {
            var result = new List<Type>();
            try
            {
                var types = GetTypesInAssembly(assembly, ignoreReflectionErrors);
                if (types != null)
                {
                    result.AddRange(GetClassesOfType(types, assignTypeFrom, onlyConcreteClasses));
                }
